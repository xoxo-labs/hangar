...
export function usePortLinks(metrics: SessionMetrics | undefined) {
  ...
  useEffect(() => {
    const refresh = () => void loadNetworkInfo(serverPort).then(setNetwork).catch(() => setNetwork(EMPTY_NETWORK))
    refresh()
  ...
  const copyPort = useCallback(async (port: number): Promise<void> => {
    try {
  ...
  const linkForPort = useCallback((port: number) =>
    shareUrl(port, links.shareHost, links.customHost, network),
  [links.customHost, links.shareHost, network])

  const isLoopbackOnly = useCallback((port: number): boolean => {
