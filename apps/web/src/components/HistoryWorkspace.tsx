...
function HistoryOverview() {
  ...
  return <div className="absolute inset-0 overflow-y-auto bg-surface-1">
    <div className="mx-auto w-full max-w-[1080px] px-[30px] py-[28px]">
      ...
      {!enabled && history.length === 0 && <Empty title="History is off" detail="Enable session history in Settings to retain local run summaries and resource timelines." />}
      {enabled && history.length === 0 && <Empty title="No previous runs" detail="Completed, failed and stopped sessions will appear here." />}
      {history.length > 0 && filtered.length === 0 && <Empty title="No matching runs" detail="Try another search or result filter." />}
...
function HistoryRow({ entry, last, onOpen }: { entry: SessionHistoryEntry; last: boolean; onOpen: () => void }) {
  ...
  return <button type="button" className={cx("group grid w-full grid-cols-[minmax(180px,1.25fr)_100px_115px_130px_18px] items-center gap-[14px] px-[14px] py-[11px] text-left hover:bg-surface-a3", !last && "border-b border-surface-4")} onClick={onOpen}>
...
function groupByDay(entries: SessionHistoryEntry[]): Array<[string, SessionHistoryEntry[]]> {
  ...
  for (const entry of entries) {
    ...
    const key = date.toDateString() === today.toDateString() ? "Today" : date.toDateString() === yesterday.toDateString() ? "Yesterday" : date.toLocaleDateString(undefined, { weekday: "long", month: "short", day: "numeric" })
    groups.set(key, [...(groups.get(key) ?? []), entry])
  }
  return [...groups.entries()]
}

function formatCpu(value: number): string { return `${value < 10 ? value.toFixed(1) : Math.round(value)}%` }
...
function formatFullDate(timestamp: number): string { return new Date(timestamp).toLocaleString(undefined, { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit" }) }
