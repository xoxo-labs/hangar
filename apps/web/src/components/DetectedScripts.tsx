...
export function DetectedScripts({
  ...
  /** Process names already taken; their rows dim and disable. */
  ...
  hint?: ReactNode
  /** The project editor collapses this in Edit mode, where processes are the errand. */
  collapsible?: boolean
  defaultOpen?: boolean
}) {
  ...
  const label = pkg ? `Detected scripts · ${pkg.manager}` : "Detected scripts"
  ...
  return (
    <div className="flex w-full flex-col items-start gap-[5px]">
      {collapsible ? (
        <button
          type="button"
          className="flex items-center gap-1 text-sm tracking-label text-surface-10 hover:text-surface-12"
          aria-expanded={expanded}
          onClick={() => setOpen((current) => !current)}
        >
          <ChevronRight
            className={cx("size-[11px] transition-transform duration-[120ms] ease-[ease]", expanded && "rotate-90")}
            aria-hidden="true"
          />
          {label}
          {!expanded && <span className="text-surface-8">· {scripts.length}</span>}
        </button>
      ) : (
