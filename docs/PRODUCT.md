...
| **Published ports (Tailscale)** | A detected Tailscale-IP link can enable a tailnet-only localhost proxy without changing its `100.x:<port>` address. Ports can also be published as **public on the internet** (`tailscale funnel`), where recipients need no account or client, or as opt-in **Tailnet HTTPS** (`tailscale serve`). All modes proxy into `localhost`, so a loopback-only dev server works untouched. Public publishing always confirms; live proxies and shares are flagged on their sidebar row, listed in the status bar, and withdrawn when the process ends or Hangar quits. |
