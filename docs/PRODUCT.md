...
| **Published ports (Tailscale)** | Publish a detected port straight from its QR dialog or the process's right-click menu: **public on the internet** (`tailscale funnel`), where whoever you send the link to needs no account and no client, or opt in under Settings to reveal **Tailnet HTTPS** (`tailscale serve`) for managed private tailnets. Both proxy from the Tailscale edge into `localhost`, so a loopback-only dev server works untouched. Publishing publicly always confirms; a live share is flagged on its sidebar row, listed with a QR trigger in the status bar, and withdrawn when the process ends or Hangar quits. |
